...
const LiveAlert: React.FC<LiveAlertProps> = ({ onResolve }) => {
  ...
  useEffect(() => {
    const loadAlert = async () => {
      try {
        const active = await api.getActiveAlert();
        if (active) {
          setAlert(active);
          runCrowdAnalysis();
        } else {
  ...
  const runCrowdAnalysis = async () => {
    ...
    if (alert) {
      try {
        await api.updateCrowdAnalysis(alert.id, {
        ...
        console.error('Failed to update crowd analysis:', error);
      }
    }
  };
  ...
  const handleResolve = async () => {
    try {
      await api.resolveAlert(alert.id, note);
      onResolve();
    } catch (error) {
  ...
  const handleCancel = async () => {
    try {
      await api.cancelAlert(alert.id);
      onResolve();
    } catch (error) {
